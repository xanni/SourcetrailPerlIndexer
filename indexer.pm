...
sub _check_global_variable {
    my ( $node, $name ) = @_;

    my $global_id = $globals{$name};
    if ($global_id) {
        my $reference_id = recordReference( $file_id, $global_id, $REFERENCE_USAGE );
        recordReferenceLocation( $reference_id, $file_id, $node->line_number, $node->column_number, $node->line_number,
            $node->column_number + length( $node->content ) - 1 );
    }

    return $global_id;
} ## end sub _check_global_variable

sub _index_global_variable {
    my ($node) = @_;
    ...
    if ( !_check_global_variable( $node, "$sigil$name" ) ) {
        ...
        recordSymbolLocation( $symbol_id, $file_id, $node->line_number, $node->column_number, $node->line_number,
            $node->column_number + length( $node->content ) - 1 );
        $globals{"$sigil$name"} = $symbol_id;
    } ## end if ( !_check_global_variable( $node, "$sigil$name"...))

    return;
} ## end sub _index_global_variable
...
sub index_symbol {
    ...
    $name = "${package}::$name" unless $name =~ m/::/x;
    return if _check_global_variable( $node, "$sigil$name" );
